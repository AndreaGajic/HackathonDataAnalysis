...
with open(Path(os.path.dirname(__file__) + filelocation), 'r') as f: #Opens the file using fileio. The Path() allows it to work on both mac and windows. __file__ returns the file location.
    ...
    for index, line in enumerate(f, start=1):
        ...
        if index != 1: #If not the matchdata list
            ...
            if not (str(teamnumber) in results): #First match
                ...
                updatedata[teamnumber][matchnum].update(matchdata.items()) #puts the match data into the match number
